...
def sparsemax(input, dim=-1):
    ...
    """
    # For numerical stability, shift input by its max.
    ...
    # Create a range tensor: 1, 2, ..., k.
    dim_size = input.size(dim)
    range_tensor = torch.arange(1, dim_size + 1, device=input.device, dtype=input.dtype)
    # Reshape for broadcasting.
    shape = [1] * input.dim()
    shape[dim] = dim_size
    range_tensor = range_tensor.view(*shape)
    # Determine the sparsity of the projection.
    support = (1 + range_tensor * z_sorted) > z_cumsum
    k = support.sum(dim=dim, keepdim=True)
    # Compute the threshold tau.
    tau = (z_cumsum.gather(dim, k.long() - 1) - 1) / k.to(input.dtype)
    # Apply the sparsemax function.
...
class SparsemaxAttention(nn.Module):
    """
    Multi-head attention layer that uses sparsemax instead of softmax.
    ...
    def forward(self, query, key, value, mask=None):
        """
        Forward pass for sparsemax multi-head attention.
        ...
        attn_output_weights = sparsemax(scores, dim=-1)
...
class SparsemaxEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, dim_feedforward, dropout=0.1):
        ...
        self.self_attn = SparsemaxAttention(d_model, n_heads)
...
class SparsemaxEncoder(nn.Module):
    def __init__(self, d_model, n_heads, num_layers, dim_feedforward, dropout=0.1):
        ...
        self.layers = nn.ModuleList([SparsemaxEncoderLayer(d_model, n_heads, dim_feedforward, dropout) for _ in range(num_layers)])
...
class TimeseriesTransformer(nn.Module):
    def __init__(self, input_feature_size, output_feature_size, d_model, nhead, num_encoder_layers, dim_feedforward):
        ...
        self.encoder = SparsemaxEncoder(d_model, nhead, num_encoder_layers, dim_feedforward)
