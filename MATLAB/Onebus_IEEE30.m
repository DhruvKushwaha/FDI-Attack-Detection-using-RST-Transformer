...
clc
define_constants;
mpc = loadcase('case30');  % Load IEEE 9-bus system
...
% Generate arr` with sine-based noise and Gaussian noise
...
att_buses = 10; % # of buses to attack
%vulnerable_buses = Jacobian_Sens(mpc, mpopt, att_buses);
vulnerable_buses = [15,18,19,20,21,23,24,26,29,30];
...
for iter = 1:num_iterations
    ...
    attack_labels = zeros(length(vulnerable_buses), 1);  % Initialize a zero vector for all buses
...
csv_filename = strcat('./Data/IEEE_case30_onebus_',string(per_attack*100)...
    ,'%.csv');
writetable(data_table, csv_filename);

fprintf('\n✅ Data saved successfully as %s!\n', csv_filename);
