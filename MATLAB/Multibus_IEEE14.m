...
clc
define_constants;
mpc = loadcase('case14');  % Load IEEE 9-bus system
...
att_buses = 5; % # of buses to attack
%vulnerable_buses = Jacobian_Sens(mpc, mpopt, att_buses);
vulnerable_buses = [4,5,9,10,14];
...
for iter = 1:num_iterations
    ...
    if ismember(iter, attack_indices)
        ...
        num_attacked_buses = randi([2, 5]);  % Choose any
...
P_from_names = strcat("From_Bus_",...
    string(mpc.branch(:, F_BUS)), "_", string(branch_numbers));
P_to_names   = strcat("To_Bus_",...
    string(mpc.branch(:, T_BUS)), "_", string(branch_numbers));
gen_numbers = (1:num_gens)';
Gen_names = strcat("Gen_Bus_",...
    string(mpc.gen(:, GEN_BUS)), "_", string(gen_numbers));
...
csv_filename = strcat('./Data/IEEE_case14_multibus_',string(per_attack*100)...
    ,'%.csv');
